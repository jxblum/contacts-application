...
package example.tests.spring.boot.geode.client;
...
public class SpringBootApacheGeodeClientApplication {
  ...
  @Bean
  ApplicationRunner assertGeodeClientCacheRegions(GemFireCache gemfireCache) {

    return args ->
      gemfireCache.rootRegions().stream()
        .map(Region::getName)
