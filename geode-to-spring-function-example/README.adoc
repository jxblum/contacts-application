...
`Function` contained in the JAR file, i.e. `SpellCheckerWithAutoCorrectFunction`, after I deployed the JAR simply by
invoking the `SpringDefinedFunctionAwareRegistrar` `Function` with the desired package argument.

Next, I execute the SDG `SpringDefinedFunctionAwareRegistrar` `Function` then, using _Gfsh_ with the following command...
...
As you can see, I would pass the package containing the SDG-defined `Functions`.
...
SDG's `SpringDefinedFunctionAwareRegistrar`, hence the `SpringDefinedFunctionInitializer` class along with
the application-specific `Function(s)` (e.g. `SpellCheckerWithAutoCorrectFunction`).

When this class extension is placed in the same package as the SDG-defined `Functions`, then the `Function` uses
its own package as a basis for the "_classpath component scan_".

However, it is possible to override the many configuration-based properties exposed by SDG's `SpringDefinedFunctionAwareRegistrar` class
in your application-specific extension (e.g. `SpringDefinedFunctionInitializer`) simply by overriding
the appropriate method.

For instance, I could have put the application-specific, `SpringDefinedFunctionInitializer` class
in the `example.app` package, and then specify the "basePackages" by overriding the `protected`,
`getBasePackages()` method...
...
Ideally, I would also have liked to allow for "_automatic registration_" of SDG-defined `Functions` to occur
at deployment-time, when using the _Gfsh_ `deploy` command.  As such, the SDG `SpringDefinedFunctionAwareRegistrar` class
...
which allows the GemFire/Geode objects (components) to receive a callback on initialization (i.e. by implementing
the `Declarable` interface's `initialize(:Cache, :Properties)` method), which the SDG `SpringDefinedFunctionAwareRegistrar`
...
(see last paragraph in the page) no longer works in Apache Geode.

If that had worked, then the SDG-defined `Functions` bundled and packaged in the JAR file would have been automatically
registered at deployment-time using the _Gfsh_ `deploy` command without any extra effort on the user's part, such as
by having to subsequently execute the `SpringDefinedFunctionInitializer` `Function` using Gfsh's
